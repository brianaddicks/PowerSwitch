function Get-HpArubaHostConfig {
    ...
    $VerbosePrefix = "Get-HpArubaHostConfig:"
    ...
    :fileloop foreach ($entry in $LoopArray) {
        ...
        $EvalParams.Regex = [regex] '^snmp-server\ contact\ ".+?"\ location\ "(.+?)"'
        $Eval = Get-RegexMatch @EvalParams -ReturnGroupNumber 1
        if ($Eval) {
    ...
    if (!($ReturnObject.IpAddress)) {
        $IpRoute = Get-HpArubaStaticRoute -ConfigArray $LoopArray
        if ($IpRoute) {
            $IpInterface = Get-HpArubaIpInterface -ConfigArray $LoopArray
